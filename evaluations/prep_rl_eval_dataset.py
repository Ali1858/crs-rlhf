...
random.seed(90)
num_samples_per_dataset = 25

def get_oasst_ds(tokenizer,conf,seed=90):
        ...
        extra_eval_ds = eval_ds.select(range(num_samples_per_dataset*3,num_samples_per_dataset*4))

        return get_records(final_eval_ds),get_records(hp_tuning_eval_ds),get_records(extra_eval_ds)


def get_alpaca_eval_ds(tokenizer):
    sampled_records = []
...
def prepare_dataset(conf):
    ...
    oasst_final, oasst_hp_tuning, oasst_extra = get_oasst_ds(tokenizer,conf,seed=90)
    alpaca_eval_ds.extend(oasst_final)
    dump_json(alpaca_eval_ds,'final')
    dump_json(oasst_hp_tuning,'hp_tuning')
    dump_json(oasst_extra,'extra')
