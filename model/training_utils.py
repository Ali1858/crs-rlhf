...
def get_sft_model(tokenizer,config,pad_vocab_size_to_multiple_of=16):
    ...
    model = transformers.AutoModelForCausalLM.from_pretrained(config["model_name"], cache_dir=config["cache_dir"],torch_dtype=config["dtype"],)
...
def get_sft_tokenizer(config,tokenizer_config):
    """Rewritten from:
    https://github.com/LAION-AI/Open-Assistant/blob/main/model/model_training/utils/utils.py#L208"""
    tokenizer_name = config["model_name"]
    assert "llama" in tokenizer_name, "current only llama model supported"

    special_tokens = tokenizer_config["llama"]

    tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_name, cache_dir=config["cache_dir"])
